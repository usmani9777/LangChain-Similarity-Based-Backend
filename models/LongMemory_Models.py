...
#     session_id: str = Field(...,min_length=10,max_length=2000,description="Session identifier")
...
class Memory(BaseModel):
    ...
    last_accessed: int = Field(default_factory=get_now_timestamp)
    
    importance: float = Field(0.0, ge=0.0, le=1.0)

    @model_validator(mode="after")
    def compute_importance(self) -> "Memory":
        # Convert the integer timestamps back to date objects for the math
        created_date = date.fromtimestamp(self.created_at)
        accessed_date = date.fromtimestamp(self.last_accessed)
        today = date.today()

        # 1️⃣ Weight by type
        type_weight = {"Personal": 0.9, "Goal": 1.0, "Fact": 0.6}[self.memory_type]

        # 2️⃣ Time decay
        days_since_created = max((today - created_date).days, 0)
        created_decay = max(0.3, 1 - (days_since_created / 365))

        # 3️⃣ Access boost
        days_since_access = max((today - accessed_date).days, 0)
        access_boost = max(0.5, 1 - (days_since_access / 30))

        # 4️⃣ Final score
        self.importance = round(type_weight * created_decay * access_boost, 3)
        return self